...
# DJF月份索引 (0-based: 5,6,7 对应 12,1,2月)
DJF_MONTHS = [11, 0, 1]
...
def get_djf_mean(ds, var_name):
    ...
    # DJF平均
    djf = var.isel(time=DJF_MONTHS).mean(dim='time')
...
for exp in ['pi'] + list(EXPERIMENTS.keys()):
    ...
    djf_data[exp]['heat_sw'] = -get_djf_mean(ds, 'surface_exchange_flux_nonadvective_sw')
    djf_data[exp]['heat_lw'] = -get_djf_mean(ds, 'surface_exchange_flux_nonadvective_lw')
    djf_data[exp]['heat_lh'] = -get_djf_mean(ds, 'surface_exchange_flux_nonadvective_lh')
    djf_data[exp]['heat_sh'] = -get_djf_mean(ds, 'surface_exchange_flux_nonadvective_sh')
    # 总和 = sw + lw + lh + sh
    djf_data[exp]['heat_total'] = (djf_data[exp]['heat_sw'] + djf_data[exp]['heat_lw'] +
                                     djf_data[exp]['heat_lh'] + djf_data[exp]['heat_sh'])

    # 盐度预算 (淡水通量引起的盐度趋势 - 读取各分项并求总和)
    djf_data[exp]['mass_seaice'] = get_djf_mean(ds, 'salt_rhs_sum_surface_ocean_flux_advective_negative_rhs_sum_sea_ice_melt')
    djf_data[exp]['mass_rain'] = get_djf_mean(ds, 'salt_rhs_sum_surface_ocean_flux_advective_negative_rhs_sum_rain_and_ice')
    djf_data[exp]['mass_snow'] = get_djf_mean(ds, 'salt_rhs_sum_surface_ocean_flux_advective_negative_rhs_sum_snow')
    djf_data[exp]['mass_evap'] = get_djf_mean(ds, 'salt_rhs_sum_surface_ocean_flux_advective_negative_rhs_sum_evaporation')
    djf_data[exp]['mass_river'] = get_djf_mean(ds, 'salt_rhs_sum_surface_ocean_flux_advective_negative_rhs_sum_rivers')

    # 加载并计算DJF平均的海冰密集度
    a_ice_djf = a_ice_data[exp]['a_ice'].isel(time=DJF_MONTHS).mean(dim='time')

    # 应用海冰mask到sea ice salt flux: a_ice=0时,mass_seaice=0
    if djf_data[exp]['mass_seaice'] is not None:
        # 创建mask: 只保留a_ice > 0的区域
        ice_mask = a_ice_djf > 0
        # 应用mask (a_ice=0的地方设为0)
        djf_data[exp]['mass_seaice'] = djf_data[exp]['mass_seaice'].where(ice_mask, 0)
        print(f'    已对 mass_seaice 应用海冰mask (a_ice > 0)')

    # 将river变量中的NaN替换为0
    if djf_data[exp]['mass_river'] is not None:
        djf_data[exp]['mass_river'] = djf_data[exp]['mass_river'].fillna(0)

    # 总淡水通量盐度趋势 = 各分项总和
    djf_data[exp]['mass_total'] = (djf_data[exp]['mass_seaice'] +
                                     djf_data[exp]['mass_rain'] +
                                     djf_data[exp]['mass_snow'] +
                                     djf_data[exp]['mass_evap'] +
                                     djf_data[exp]['mass_river'])

    # 组合通量
    djf_data[exp]['heat_radiation'] = djf_data[exp]['heat_sw'] + djf_data[exp]['heat_lw']
    djf_data[exp]['heat_turbulent'] = djf_data[exp]['heat_lh'] + djf_data[exp]['heat_sh']

    # other FW = rain + snow + evap + river
    djf_data[exp]['mass_other'] = (djf_data[exp]['mass_rain'] +
                                     djf_data[exp]['mass_snow'] +
                                     djf_data[exp]['mass_evap'] +
                                     djf_data[exp]['mass_river'])
...
for exp in ['pi'] + list(EXPERIMENTS.keys()):
    ...
    a_ice_djf = a_ice_djf.squeeze().values  # (180, 360)
    ice_mask = a_ice_djf > 0  # 海冰存在的区域

    for var in ['heat_total', 'heat_radiation', 'heat_turbulent',
                'mass_total', 'mass_seaice', 'mass_other']:
        # 获取数据并确保是2D: (180, 360)
        data_vals = djf_data[exp][var].values
        if data_vals.ndim > 2:
        ...
        # 添加循环点
        djf_cyclic[exp][var], lon_cyclic = add_cyclic_point(data_smooth, lon)
...
for exp in EXPERIMENTS.keys():
    ...
    for var in ['heat_total', 'heat_radiation', 'heat_turbulent',
                'mass_total', 'mass_seaice', 'mass_other']:
        anomalies[exp][var] = djf_cyclic[exp][var] - djf_cyclic['pi'][var]
...
print('\nPI绝对值范围:')
for var in heat_vars:
    pi_vals = djf_cyclic['pi'][var][mask_so]
...
output_file1 = 'figures/surface_heat_budget_anomalies_djf.pdf'
...
output_file2 = 'figures/surface_freshwater_budget_anomalies_djf.pdf'
