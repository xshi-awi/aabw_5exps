#!/usr/bin/env python
"""
Build the LaTeX response letter, following the format of SUBMISSION/example_response.tex
but using the Nature Communications class (sn-jnl.cls), which is what this manuscript
is submitted under.

Conventions taken from the example:
  reviewer comment   plain black text, reproduced verbatim and in full
  our reply          \textcolor{blue}{\textbf{...}}
  quoted manuscript  \textcolor{blue}{\textbf{\textit{"..."}}}

Figures are numbered per reviewer (R1.1, R2.1, ...) and the same underlying figure
may appear under more than one reviewer, because several reviewers asked the same
question and should not have to look at another reviewer's section.
"""
...
# ----------------------------------------------------------------- helpers
def reply(txt):
    return '\\textcolor{blue}{\\textbf{' + txt + '}}'


def quote(txt):
    return '\\textcolor{blue}{\\textbf{\\textit{``' + txt + "''}}}"
...
def fig(path, caption, label, width='\\textwidth'):
    return (f'\n\\begin{{figure}}[htbp]\n\\centering\n'
...
# ================================================================= preamble
add(r"""\documentclass[pdflatex,sn-nature]{sn-jnl}
...
1.84~Sv (Fig.~\ref{fig:r1domain})."""), MED)

add(fig('letter_figs/figR1_wmt_domain_sensitivity.png',
        r"""\textbf{Figure R1.1} (manuscript Fig.~8). Transformation integrated south of
...
contrast between thermally dominated interglacials and haline dominated glacials is unchanged.""",
        'fig:r1domain'))

add(reply(r"""Second, and more importantly, the density range matters more than the domain.
...
add(reply(r"""Third, we now state the residual model limitation explicitly rather than alluding to
it. We diagnosed directly where the model forms its dense water (Fig.~\ref{fig:r1conv})."""), MED)

add(fig('letter_figs/figR6_convection_sites.png',
        r"""\textbf{Figure R1.2} (new analysis). (a--e) Winter mixed layer depth for the five
...
add(reply(r"""This is open-ocean convection, not the shelf overflow pathway that operates in the
...
add(r'\textbf{Boundary conditions}', MED)

add(comment(r"""The description of the prescribed boundary conditions for the paleoclimate simulations could be expanded. In particular, the glacial simulations use the GLAC-1D ice-sheet reconstruction, but it is not entirely clear how the ice-sheet changes are implemented in the model (e.g., topography, land-sea mask, ice-sheet extent, bathymetry, or other boundary conditions). As Antarctic dense water formation is highly sensitive to such factors, providing additional details on the experimental setup would help readers assess the robustness of the simulations.
...
A summary table listing the prescribed greenhouse gas concentrations, orbital parameters, and ice-sheet boundary conditions for each experiment would also improve the readability of the Methods section."""), MED)

add(reply(r"""Added. Table~1 of the revised Methods now lists the greenhouse gas concentrations,
...
surrounding text to describe how GLAC-1D enters the model, which we agree was underspecified:"""), MED)

add(quote(r"""The GLAC-1D reconstruction enters the model through several boundary fields
...
add(r'\textbf{Clarifying several interpretations}', MED)

add(comment(r"""Most interpretations are convincing, although a few statements could be presented more carefully or supported more explicitly. For example, some descriptions of the role of the Southern Annular Mode appear stronger than the presented evidence suggests, and several discussions would benefit from more direct references to the relevant figure panels."""), MED)

add(reply(r"""We have moderated the Southern Annular Mode claims. The most overstated passage was
...
after 1000 years, so the simulated ages are lower bounds. Both are now stated in the text."""), MED)

add(r'\textbf{Figure labelling and subpanel references}', MED)

add(comment(r"""Please consider adding latitude and longitude labels to all map figures where appropriate. In addition, referring to specific figure subpanels throughout the text, rather than only the full figure number, would make the discussion easier to follow."""), MED)

add(reply(r"""Subpanel references have been added throughout the Results and Discussion. The polar
...
versions. We are happy to add complete labelling if the reviewer prefers."""), MED)

add(r'\subsection*{Specific comments}', MED)

R1 = [
 (r"""1. ' LIG, LGM and MIS should be capitalised.""",
  r"""Corrected throughout."""),
 (r"""2. Line 21-22: The transition to SAM is quite abrupt.""",
  r"""The abstract now introduces the Southern Annular Mode through the mechanism that connects it
to the mean state, noting that because the balance between the thermal and haline pathways is set
by how much of the surface is ice covered, it also governs the response to internal atmospheric
variability."""),
 (r"""3. Line 24: "same mechanisms" sounds vague her without additional context. The abstract should be understandable on its own without requiring the reader to refer to the main text.""",
  r"""Replaced with an explicit statement. The abstract now reads that the mode works ``through heat
loss when the ocean is open and through brine rejection when it is ice covered''."""),
 (r"""4. Lines 25-27: Similar to Comment 2 above, additional context is needed before introducing the deep ocean ventilation results. The motivation for analysing ventilation ages only becomes clear later in the manuscript, whereas the abstract should be self-contained.""",
  r"""The abstract now bridges from surface forcing to ventilation via the overturning circulation,
...
analysis requested by Reviewer 3."""),
 (r"""5. Lines 102-105: SAM is introduced only briefly here. Given its importance throughout the Results and Discussion, please consider providing a slightly more detailed introduction.""",
  r"""The introduction now defines the mode as the leading mode of extratropical Southern Hemisphere
...
bottom water changes."""),
 (r"""6. Line 125: typo: 'more'""",
  r"""Corrected (``moer'' to ``more'')."""),
 (r"""7. Line 127: Figure numbering; A3 can't be called before A1 and A2.""",
  r"""The appendix figures have been reordered so that they are cited in numerical order."""),
 (r"""8. Line 129: 'Here,'""",
  r"""Corrected."""),
 (r"""9. Line 140: As a result, the deep MLD signals, characteristic of open-ocean convection, largely.... Also, "vanish" is perhaps not the most scientific wording.""",
  r"""Rephrased to ``are strongly reduced and become confined to narrow coastal bands and
polynyas'', and we now quantify the reduction rather than describing it qualitatively."""),
 (r"""10. Line 141: These signals are not particularly visible even in the coastal regions (the darker blue shading in LGM is at ~200m, so considerably shallower than the interglacials).""",
  r"""This is a fair objection and we have addressed it with numbers. Summed south of 55$^\circ$S,
...
so the reviewer's reading of the shading is correct and is now reflected in the text."""),
 (r"""11. Line 134-141: Fig.1 i-l subpanels are not discussed at all.""",
  r"""A paragraph discussing the surface density panels has been added. It notes that the glacial
...
interglacials, against 15--30\% for the haline term, and falls to 12--17\% in the glacial states."""),
 (r"""15. Fig. 2 Would it be possible to share a figure with the same y-axis limits for all panels? This may not be necessary for the final manuscript, but it would make it easier to compare the magnitude of the changes across the different climate states.""",
  r"""Yes. Supplementary Fig.~S5, reproduced as Fig.~\ref{fig:r1axis} below, shows the same
...
the main text now points to the common-axis version explicitly."""),
 (r"""16. Line 174: "Section 4?"If this is intended to refer to the Methods section""",
  r"""Corrected to refer to the Methods section by name."""),
 (r"""17. Fig.3 Please consider adding the climate-state labels at the top of each column, as in the other figures.""",
  r"""Added, here and on the other multi-panel figures. Reviewer 3 made the same request."""),
 (r"""18. Line 180/181: The LIG is first described as "showing modest changes", but later as "exhibiting a pronounced increase". These descriptions appear inconsistent.""",
  r"""The two statements referred to different quantities, which was not clear. The changes are
modest in the circumpolar integral and pronounced locally in the Ross Sea sector. The text now says
so explicitly."""),
 (r"""19. Could the authors briefly explain what is meant by thermal density tendency and sea-ice-driven density tendency in the methods.""",
  r"""Definitions have been added to the Methods:"""),
 (r"""20. Line 209: Please add the appropriate reference(s) here to support this statement.""",
  r"""We now cite Cerove\v{c}ki et al. (2013) and Abernathey et al. (2016) in support of the
statement that turbulent rather than radiative fluxes control the surface buoyancy budget."""),
 (r"""21. Line216-246: Please refer to the relevant subpanels of Figure A5""",
  r"""Done."""),
 (r"""22. Line 249, 255: The phrase "across at high latitudes" is somewhat unclear. Does this refer specifically to the Antarctic coastal region or the Southern Ocean south of a particular latitude? Please consider rephrasing for clarity.""",
  r"""Both instances have been rewritten. They now read ``over the ice-free parts of the Southern
Ocean south of about 50$^\circ$S'' and ``south of the ice edge'' respectively."""),
 (r"""23. Could the authors clarify where the primary deep water formation regions occur in this model? Identifying these regions would also help assess whether the SAM-related changes shown in Figure 5 are directly influencing the main regions of dense water formation, as relatively little signal is apparent over the Weddell and Ross Seas.""",
  r"""We have added this diagnosis to the Results; it is shown in Fig.~\ref{fig:r1conv} above. In the
...
though the transformation response is not."""),
 (r"""24. Lines 261-263: Please consider expanding on the physical mechanism linking positive SAM to freshwater loss and intensified brine rejection by additional explanation and supporting references.""",
  r"""Expanded, with the mechanism stated as a sequence: strengthened and poleward-shifted westerlies
...
References added."""),
 (r"""25. Line 291, 294: Please check citation format""",
  r"""Corrected."""),
 (r"""26. Line 301:Figure A4 appears to show only model results. Please consider including the observational estimates in the figure (or referring to them directly) to support this statement. As noted above, it would also improve readability if the appendix figures were cited in numerical order.""",
  r"""We have added the quantitative comparison to the text and to the new Fig.~8, reproduced as
...
numerical order."""),
 (r"""27. Lines 308-321: The discussion quantifies the interglacial changes (e.g., lines 315-316), but similar quantification is not provided for the glacial changes. For consistency, please consider including quantitative estimates for both.""",
  r"""Added. The glacial transformation maximum weakens to below 60~Sv and moves to
$\sigma_2 = 37.0$--$37.5$~kg~m$^{-3}$, with the thermal share falling to 12--17\%."""),
 (r"""28. Lines 326 appear to repeat the statement made in line 323. In addition, the general statement in lines 323-325 does not appear to hold equally across all climate states. For example, the Ross Sea and Weddell Sea exhibit different behaviour for the peak-density classes.""",
  r"""The repetition has been removed by merging the two sentences. We have also added the
qualification the reviewer asks for, noting that the Ross Sea departs from the circumpolar picture
and that in the glacial states the signal is carried largely by the Weddell sector."""),
 (r"""29. Line 339: Please provide a reference for the sediment-record evidence discussed here.""",
  r"""On reflection we think the passage was speculative rather than supported, and Reviewer 3 raised
...
a practical route to a marine reconstruction."""),
 (r"""30. Section 4.1: Please define the full names of the abbreviated components""",
  r"""Done. The Methods now spell out that the radiative component is the sum of the shortwave and
longwave fluxes and the turbulent component the sum of the latent and sensible heat fluxes."""),
 (r"""31. Line 388: Please check the citation formatting.""",
  r"""Corrected."""),
 (r"""32. Lines 394-403: Please consider including a summary table listing the prescribed boundary conditions for each experiment (e.g., greenhouse gas concentrations, orbital parameters, ice-sheet reconstruction, and any other relevant forcings). This would make the experimental setup considerably easier to follow.""",
  r"""Added as Table~1 of the revised manuscript; please see our reply to the general comment on
...
comparable between panels, which the independently scaled main-text version does not allow.""",
                'fig:r1axis'))
    if c.startswith('19.'):
        add(quote(r"""We refer throughout to the thermal density tendency, meaning the surface
...
density, whereas melting releases freshwater and reduces it."""), MED)

add(r'\clearpage', '\n')


# ================================================================= REVIEWER 2
add(r'\section*{Reviewer \#2}', MED)

add(comment(r"""General
...
So in principle this is an interesting and topical paper and I would like to be able to support its publication. However, I have two big issues with the present MS that in my view mean it needs at least some substantial revision."""), MED)

add(reply(r"""We are grateful for both issues, which were well aimed, and we address them in turn
below."""), MED)

add(r'\subsection*{Major issue 1}', MED)

add(comment(r"""1) There is a mismatch with observational studies of the modern ocean. In their model the pre-industrial situation is dominated by thermal buoyancy forcing whereas Pellichero et al, (2018) find the opposite: The title of their observation-based paper makes this clear: "Southern Ocean meridional overturning is driven by freshwater fluxes". Shi et al dismiss this difference in a single sentence, saying it is due to "model biases or regional differences in analysis domains" (line 305). Comparing their Fig 3a, Fig 4a with Pellichero Fig 2a and c, the regions look similar, suggesting it's mostly model bias. But the switch from thermal to haline forcing is the present paper's main result, so if indeed it is due to model bias, this seems like a major problem."""), MED)

add(reply(r"""The reviewer is right that the original single sentence was inadequate, and right to
...
unfavourable to us."""), MED)

add(reply(r"""\textbf{The domains are less similar than they appear.} Pellichero et al. define their
...
add(reply(r"""\textbf{The density range matters more than the domain.} This was the more
...
bottom water densities, and where they overlap they agree in sign."""), MED)

add(reply(r"""\textbf{A residual discrepancy remains, and it is genuine model bias.} We can rule out
...
area and the maximum mixed layer depth both contract sharply in the glacial states.""",
        'fig:r2conv'))

add(reply(r"""Open-ocean convection exposes a large area to the atmosphere and so recruits an
...
add(reply(r"""\textbf{Does the main result survive?} We think it does, for two reasons that we now
...
as new in that respect."""), MED)

add(reply(r"""What the transformation analysis adds is of two kinds. The first is measurement rather
...
add(reply(r"""The second is scope. Their simulations are ocean-only, with a prescribed atmosphere,
...
rather than as a claim that they missed something."""), MED)

add(r'\subsection*{More minor points}', MED)

add(comment(r"""The fundamental issue must be getting right the formation of sea ice and rejection of brine close to the continent, followed by the mixing of that dense water with the surroundings, all processes that intrinsically occur at fine spatial scales. In this regard the model looks good, by comparison to most paleo studies. The comparatively high resolution close to the continent must be helpful I'm sure, (though at 25 km it may still not be enough to realistically model the dynamics of polynya formation and brine rejection). Some more information on just how this is being managed would add to the value of the paper."""), MED)

add(reply(r"""We agree with the reviewer's caveat and have expanded the Methods accordingly:"""), MED)

add(quote(r"""At this resolution the larger coastal polynyas are represented as features, but the
...
floating ice shelves, and the basal melt it supplies, are absent from the simulations."""), MED)

add(comment(r"""The authors highlight the lack of deep convection in the glacial open ocean due to the very extensive winter-time ice cover. A little more discussion of how/why the model achieves this ice cover would be welcome, as previous paleo-model studies often don't get that, and as a result may have too much deep convection in glacial simulations, which doesn't match so well with paleo-proxies. (Another paper they don't cite is Lhardy, Climate of the Past, 17, 1139-1159, 2021, which discusses how most PMIP models fail in this regard.)"""), MED)

add(reply(r"""Thank you for this reference, which we have added and discussed. We now note that
...
quantitative agreement remains uncertain."""), MED)

add(comment(r"""In Results section 2.1 the acronyms (MH, LIG etc) are used without introduction. They are defined later, section 4, but as the paper is laid out this is not obvious to the reader coming to section 2."""), MED)

add(reply(r"""Corrected. The five experiments and their abbreviations are now introduced at the
start of the Results, at first use, in addition to the Methods definition. Reviewer 3 made the same
point."""), MED)

add(comment(r"""The discussion of the role of the effects of the SAM is interesting, but could I think be shortened, since it is somewhat secondary to the main messages of the paper. Figs 5 and 6 could be moved to the supplementary information."""), MED)

add(reply(r"""We have done this. All of the Southern Annular Mode figures, including Figs 5 and 6,
...
add(comment(r"""For figs 5 and 6, I think it would be helpful if the forcings were on the same equivalent scales of buoyancy anomaly -- currently they are in different units and it is impossible to see how they compare in absolute values."""), MED)

add(reply(r"""This is a fair criticism. We have added a statement of the comparison in equivalent
...
add(reply(r"""In the pre-industrial simulation the Weddell sector accounts for 70\% of the area with
...
no model showing convincing shelf export in the Weddell Sea."""), MED)

add(reply(r"""We note in passing, and with no criticism intended, that Toggweiler and Samuels (1995)
...
rather than from inadequate boundary conditions is directly relevant to our case."""), MED)

add(comment(r"""*In the discussion (lines 302 - 305) you mention the difference between your results in the PI (thermally dominated) and the observations (salinity dominated) may come from assessing different regions. This could be tested by comparing your method only in the regions with observations. Alternatively you could apply the WMT framework to a reanalysis product e.g. Glorys, across the Southern Ocean to provide an 'observational' estimate similar to your PI analysis.
https://data.marine.copernicus.eu/product/GLOBAL\_MULTIYEAR\_PHY\_001\_030/description"""), MED)

add(reply(r"""We took the first of these suggestions. In summary: 42.7\% of our published domain lies
...
add(fig('letter_figs/figR1_wmt_domain_sensitivity.png',
        r"""\textbf{Figure R3.2} (manuscript Fig.~8). The like-for-like comparison the reviewer
...
conclude the regime shift is not an artefact of the biased pre-industrial end member.""",
        'fig:r3domain'))

add(reply(r"""We did not carry out the reanalysis calculation. Doing it properly would require
...
have manufactured it."""), MED)

add(reply(r"""More fundamentally, the mechanism is the ice cover rather than the convection style.
...
add(comment(r"""I find the SAM wind results interesting and I think more generalisable. The results suggest changes in wind shift/strength impacts not just the ekman divergence i.e. upwelling of deepwaters, but also the salt pump via sea ice export, and thus the formation of deepwaters. This suggests changes in the wind patterns effect both the push and pull driving the overturning."""), MED)

add(reply(r"""We are grateful for this observation, which identified a mechanism our original
...
and it has become one of the more informative results in the revision."""), MED)

add(reply(r"""We separated the two routes explicitly. For the same high and low SAM composites, we
...
which measures the pull. Both were computed for all five climate states."""), MED)

add(fig('letter_figs/figR4_sam_push_pull.png',
        r"""\textbf{Figure R3.3} (new Supplementary Figure). (a) The positive phase strengthens the
...
anomaly in the peak transformation rate, with the density class of the maximum indicated.""",
        'fig:r3pp'))

add(reply(r"""Three results follow, and all three are new relative to the submitted version."""), MED)

add(reply(r"""The push and the pull are not independent expressions of the same forcing but two
...
and we can now state it quantitatively."""), MED)

add(reply(r"""The balance between the routes is state dependent, and in a way we did not anticipate.
...
response to internal variability."""), MED)

add(reply(r"""The spatial patterns show the mechanism directly."""), MED)

add(fig('letter_figs/figR5_sam_wind_ice.png',
        r"""\textbf{Figure R3.4} (new Supplementary Figure). Zonal wind stress anomaly (top) and sea
...
melts. The dipole sits further north and is substantially stronger in the glacial states.""",
        'fig:r3wi'))

add(comment(r"""It is worth noting that available estimates suggest a equatorward and weakening of the glacial westerlies, that the models do not seem to capture under glacial forcings (e.g. Gray et al 2023). Such a shift may be worth discussing in light of the 'SAM' wind mechanism, and given such a shift/weakening is absent in your glacial simulations."""), MED)

add(reply(r"""The reviewer is correct and we now discuss this. We measured our own jet: the winter
...
therefore of the opposite sign, and we state this in both the Results and the Discussion."""), MED)

add(reply(r"""Two considerations bound the consequences, and we give both. The discrepancy is shared
...
than as a reconstruction of glacial wind-driven variability."""), MED)

add(comment(r"""Interestingly the westerlies seem to intensify in both the glacial and interglacial simulations; why is this? Could this relate to the initializing simulation not being at equilibrium? Is the PI sim extended for the same additional time as the other simulations to adjust for model drift?"""), MED)

add(reply(r"""The two intensifications have different origins, and we now explain both. The LIG
...
imposed by the expanded ice sheets and sea ice."""), MED)

add(reply(r"""On drift: MH is indistinguishable from PI in both jet latitude (45.7$^\circ$S in both)
...
the upper cell is largely maintained.""",
        'fig:r3moc'))

add(reply(r"""The abyssal cell weakens from $-10.0$~Sv in PI, $-10.1$~Sv in MH and $-9.2$~Sv in LIG
...
is now shown in the new Fig.~7, as basin-mean vertical profiles and zonal-mean sections."""), MED)

add(fig('letter_figs/figR3_age_global.png',
        r"""\textbf{Figure R3.6} (manuscript Fig.~7). Basin-mean ideal age profiles (a--d), mean age
...
Atlantic, which remains ventilated from the north, changes comparatively little.""",
        'fig:r3age'))

add(reply(r"""Below 2000~m the ageing is strongly basin dependent: the Atlantic increases only
...
abyssal cell weakening while the upper cell is maintained."""), MED)

add(reply(r"""On equilibration, the reviewer is right and we now state the caveat explicitly: each
...
case. The introduction has been corrected accordingly."""), MED)

add(comment(r"""Overall, I would encourage the authors to think about what is generalisable from there results given the substantial model bias in the PI in the specific process that is being examined, or demonstrate that the bias arise from differences in the regional comparison with observations and thus apply to the real ocean."""), MED)

add(reply(r"""We hope the combination of the domain test, the density-class comparison and the
...
absolute rates of bottom water formation."""), MED)

add(r'\subsection*{Minor comments}', MED)

R3 = [
 (r"""MH, LIG are not abbreviation that commonly used so I would remove them from the text and spell out each time. Also I don't think these were not defined at any point of the manuscript.""",
  r"""The abbreviations are now defined at first use in the Results as well as in the Methods. We
...
read. If the reviewer feels strongly we will expand them throughout."""),
 (r"""L19:'in the model' needs to be clearly stated as this is not how AABW forms in the real modern ocean""",
  r"""Added to the abstract, which now reads ``In the model, interglacial dense water formation is
driven mainly by turbulent heat loss\ldots''. The same qualification has been added in the
Results."""),
 (r"""L25: change deep from "clear imprint on deep-ocean ventilation" to abyssal Southern Ocean. As it is only one depth plot at 4000m has been shown. The deep ocean is usually considered as all the water below 1000m. Plots of overturning/ideal age globally would be helpful to assess the overall impact on ventilation.""",
  r"""The abstract now refers to the abyssal overturning cell and abyssal water specifically, and the
new global age figure (Fig.~\ref{fig:r3age}) covers the whole water column rather than a single
level."""),
 (r"""L102-105: a slightly expanded intro on the SAM may be helpful""",
  r"""Expanded; please see our reply to Reviewer 1, comment 5."""),
 (r"""L 125: typo, more.""",
  r"""Corrected."""),
 (r"""L129: it would be much more useful to show the salinity minus the whole ocean change applied, to see what arises from local dynamics""",
  r"""This is a good suggestion. The glacial salinity anomalies include the uniform global increase
...
correctly."""),
 (r"""L134: 400m is not very deep given you are comparing to what is happening to ideal age at 4000 m""",
  r"""Agreed; the 400~m contour marks active convection rather than the depth reached by the
...
circulation directly rather than by implication."""),
 (r"""L144: the westerlies appear to increase under both glacial and last interglacial forcings, why?""",
  r"""Answered above, under the corresponding main comment."""),
 (r"""L152: it needs to be clearer that this is in the model""",
  r"""Rephrased to ``the model produces deep mixed layers\ldots''."""),
 (r"""L175: I wonder if this result isn't more applicable to AAIW/SAMW formation?""",
  r"""The reviewer's intuition is borne out by the density-class analysis described above. Our
...
useful reframings in the revision."""),
 (r"""L195: it the real ocean freshwater input from icesheet melting is important today (i.e. Toggweiler 1995)""",
  r"""Added, with the explicit acknowledgement that this configuration has no ice shelf cavities and
applies no prescribed meltwater flux, so that pathway is absent from our simulations."""),
 (r"""L206-209: should be moved to the introduction maybe ?""",
  r"""Moved."""),
 (r"""L291-297 : this point seems fundamental to the rest of the manuscript - the PI sim doesn't seem to capture how AABW actually forms""",
  r"""Agreed; addressed at length above."""),
 (r"""L302 - 305 : can't you just compare you model results directly in the regions with observations to test the idea that the discrepancy comes from comparing different regions?""",
  r"""Done; please see Fig.~\ref{fig:r3domain} and the accompanying discussion above."""),
 (r"""L339: this would be great, but sed rates around the Antarctic margin are very low - its hard enough getting cores with a glacial interglacial cycle that survives bioturbation""",
  r"""Agreed, and the passage has been replaced with a statement of that difficulty. Reviewer 1 asked
for a supporting reference for the same passage; we concluded the proposal was not well founded and
have withdrawn it."""),
 (r"""L347: not the water, the carbon in the water - given the substantial and variable preformed 14C aging (mentioned just after)""",
  r"""Corrected. The text now refers to the radiocarbon age of dissolved inorganic carbon."""),
 (r"""General comment on the figures : would it be possible to show whats happening under the ice shelves? At the moment they are just whited out. Anyway to show this?""",
  r"""Unfortunately not. The model configuration does not include ice shelf cavities, so there is no
...
melt is not represented."""),
 (r"""Please be consistent across figures and label MH, LIG, LGM , MIS3 on top of all panels.""",
  r"""Done throughout. Reviewer 1 made the same request."""),
]
...
# ================================================================= REVIEWER 4
add(r'\section*{Reviewer \#4}', MED)

add(comment(r"""I co-reviewed this manuscript with one of the reviewers who provided the listed reports. This is part of the Nature Communications initiative to facilitate training in peer review and to provide appropriate recognition for Early Career Researchers who co-review manuscripts."""), MED)

add(reply(r"""We thank Reviewer 4 for their contribution to the review, and the journal for
supporting early-career researchers in peer review."""), MED)

add(r'\clearpage', '\n')

# ================================================================= EDITORIAL
add(r'\section*{Editorial requirements}', MED)

add(reply(r"""\textbf{Colour vision deficiency.} The new figures use the Okabe-Ito
...
add(reply(r"""\textbf{Data availability.} A statement has been added. The model output needed to
...
acceptance. Source data for the line-graph figures are provided with the paper."""), MED)

add(reply(r"""\textbf{Code availability.} A statement has been added. The analysis and plotting
...
\url{https://github.com/xshi-awi/aabw_5exps}."""), MED)

add(reply(r"""\textbf{Source Data.} Provided for the line-graph figures, as a single spreadsheet
with one sheet per figure panel."""), MED)

add(reply(r"""\textbf{ORCID.} The corresponding author has linked an ORCID to the submission system,
and co-authors have been asked to do the same."""), MED)

add('\n\\end{document}\n')

OUT.parent.mkdir(parents=True, exist_ok=True)
OUT.write_text('\n'.join(P))
print(f'wrote {OUT}, {len(P)} pieces')
