...
These thermodynamic changes are accompanied by shifts in atmospheric forcing (Fig. \ref{sst}m-p). While the MH shows moderate changes in zonal wind, the LIG displays localized intensification (Fig. \ref{sst}n). Both LGM and MIS3 exhibit strong positive anomalies in zonal wind speed and wind stress ($>0.04$~N/m$^2$) across the circumpolar belt (Fig. \ref{sst}o,p; Fig. \ref{wind}g,h). This intensified wind stress enhances Ekman transport and facilitates the northward expansion of sea ice, fundamentally altering the air-sea interaction interface. In the glacial states the sea ice response to this wind intensification is in fact fairly uniform around the continent. Averaged between 60 and 75°S, winter ice concentration increases by 0.62 in the Ross sector, 0.70 in the Weddell sector and 0.53 in the Ad\'elie sector at the Last Glacial Maximum, and all three sectors cool to within about 0.1~K of the surface freezing point, so ice growth is no longer limited by the surface heat content anywhere. The Ross sector instead stands out in the last interglacial, where the surface ocean between 60 and 75°S is 0.97~K warmer than in PI and sits 2.72~K above the freezing point, against changes below 0.3~K in the other two sectors. Winter ice concentration there falls by 0.19, an order of magnitude more than the Weddell or Ad\'elie response. In that state the wind can export ice efficiently but the water is too far from freezing for the exported ice to be replaced, so the sector loses ice cover and gains exposure to the atmosphere. This asymmetry is why the Ross sector departs from the circumpolar pattern in the variability analysis below.
...
%% TODO before submission: acknowledgements and funding text.
%% TODO before submission: verify the four Sidorenko et al. (2019) numbers in the
%%   Discussion (about 10 Sv abyssal cell, 1.43 K SST RMSE against PHC, 0.25 m
%%   September Weddell ice thickness, and the "reasonable density distribution"
%%   assessment) against the published PDF. No copy of that paper is on the
%%   analysis machine and the project notes disagree about whether the numbers
%%   were ever checked against the full text, so they are unconfirmed here.
