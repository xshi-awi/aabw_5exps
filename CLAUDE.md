...
### 2026-09-21 (深海平衡态诊断 + 回复信三处修正)

**深海 trend 算完了，结果对我们不全是好消息，已如实写进稿子。**
脚本 `calc_deep_ocean_trend.py`（读 `production/{exp}_age/outdata/fesom/` 逐年 3D 温盐，
每个实验 396-616 年），2000 m 以下体积加权：

| exp | 分析期趋势 K/century | 后半段/前半段趋势比 | 整段记录深海升温 |
|-----|------|------|------|
| PI | 0.009 | 0.41 | - |
| MH | 0.016 | 0.18 | - |
| LIG | 0.007 | 0.13 | - |
| **LGM** | **0.113** | **1.08** | **+0.58 K** |
| **MIS3** | **0.160** | **1.37** | **+0.89 K** |

间冰期深海已经平了而且还在减速；**两个冰期没有，MIS3 甚至在加速**。原因是冰期实验从
更早的冰期态初始化而非从 PI 分叉。Methods 里全部如实写了，并说明影响范围：
ideal age 本来就已声明是下界；表面 WMT 诊断对表面强迫是年际响应，不受这种慢速深海调整影响。
图 `figures/figR10_deep_trend.pdf`（`plot_deep_trend.py`）。

**用户检查回复信后指出三个问题，都属实，已修**：
1. 13 条审稿意见被我用省略号截断过 —— 现已从 comments.txt 程序化还原为逐字全文，零省略号
2. 两处该配图没配（G5 的标注示例、comment 17 的 Fig.3）—— 补图时发现 **Fig.3/Fig.4 确实
   从来没有气候态列标签**，审稿人没记错，已改 `plot_surface_density_tendency_from_raw_data_jja.py`
   给两张图加列标题并重画
3. 图号有重复的 R1.3，且一处交叉引用在重编号后指错 —— 已全部按文档顺序重排

**新增 `nc_paper/check_letter.py`，每次改完必跑**：查图号是否连号、交叉引用是否都有对应、
标号数与图片数是否相等、有无省略号、引文能否追溯回 quotes.json。
这次就是它逮到「drift 引文因为我在中间插了新段落而失效」的问题。

**当前体量**：正文 46 页，追踪版 46 页 148 处标记，回复信 markdown 37 页 / LaTeX 42 页零错误 /
docx 161 蓝 19 图。三个格式均由 RESPONSE_LETTER.md 单一来源生成。

**Outstanding**: 致谢与基金信息、Zenodo DOI。
